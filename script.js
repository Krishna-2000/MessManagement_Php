...
function onGetMessCuts(data, id) {
  ...
  if (myObj.data.length == 0) {
    ...
    txt +=
      "<br><ion-card ><ion-card-container><ion-list><ion-card-header><ion-item><ion-col>From</ion-col><ion-col>To</ion-col><ion-col>No of Days</ion-col></ion-item></ion-card-header>";
    for (i = 0; i < myObj.data.length; i++) {
      txt +=
        "<ion-item lines='none' ><ion-col>" +
        myObj.data[i].from_date +
        "</ion-col><ion-col>" +
        myObj.data[i].to_date +
        "</ion-col><ion-col>" +
        myObj.data[i].no_of_days +
        "</ion-col></ion-item>";
    }
  }
...
function onGetMyMessCuts(data) {
  ...
  if (myObj.data.length == 0) {
    ...
    txt +=
      "<br><ion-card ><ion-card-header id='y'><ion-item id='x'><ion-col>S No</ion-col><ion-col>From</ion-col><ion-col>To</ion-col><ion-col>No of Days</ion-col><ion-item></ion-card-header><ion-list>";
    for (i = 0; i < myObj.data.length; i++) {
      txt +=
        "<ion-item lines='none' ><ion-col>" +
        (i + 1) +
        "</ion-col><ion-col>" +
        formatDate(myObj.data[i].from_date) +
        "</ion-col><ion-col>" +
        formatDate(myObj.data[i].to_date) +
        "</ion-col><ion-col>" +
        myObj.data[i].no_of_days +
        "</ion-col></ion-item>";
    }
...
function onGetExtraList(data, id) {
  ...
  if (myObj.data.length == 0) {
    ...
    txt +=
      ...
      sum +
      ".00" +
      "</ion-col></ion-item></ion-card-footer></ion-list></ion-card-container></ion-card>";
  }
  document.getElementById(id).innerHTML = txt;
}
...
function onGetGuestList(data, id) {
  ...
  if (myObj.data.length == 0) {
    ...
    txt +=
      "<br><ion-card><ion-card-container><ion-list><ion-card-header><ion-item><ion-col><strong>S.No</strong></ion-col><ion-col><strong>Guest Name</strong></ion-col><ion-col><strong>Guest Roll No</strong></ion-col></ion-item></ion-card-header>";
    for (i = 0; i < myObj.data.length - 1; i++) {
      txt +=
        "<ion-item lines='none' ><ion-col>&emsp;" +
        (i + 1) +
        "." +
        "</ion-col><ion-col>&emsp;" +
        myObj.data[i].name +
        "</ion-col><ion-col>" +
        myObj.data[i].rollno +
        "</ion-col></ion-item>";
    ...
    txt +=
      ...
      "</ion-col><ion-col>" +
      myObj.data[i].rollno +
      "</ion-col></ion-item></ion-list></ion-card-container></ion-card>";
  }
  document.getElementById(id).innerHTML = txt;
}
...
function getGuests() {
  ...
  xmlhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (myObj.data.length == 0) {
        txt +=
          "<p id='noMatch' style='text-align:center;display:none;'>No matching students</p>";
      } else {
        txt +=
          "<br><ion-card style='width:700px'><ion-card-container><ion-list><ion-card-header><ion-item --ion-color-primary='#8f15f4'><ion-col>S.No</ion-col><ion-col>Guest Name</ion-col><ion-col>Guest Roll No</ion-col></ion-item></ion-card-header>";
        for (i = 0; i < myObj.data.length - 1; i++) {
          txt +=
            "<ion-item lines='none' ><ion-col>" +
            (i + 1) +
...
function onGetProfile(data, id, isOpeningModal) {
  ...
  document.getElementById(
    id
  ).innerHTML = `<ion-card><ion-card-container><br><ion-list lines="none"><ion-item><ion-col size=4.5><ion-label> Name    
                                </ion-label>
                                </ion-col>
                                <ion-col size=2>
                                    :
...
function onMessCut(data, id) {
  ...
  if (myObj.added) {
    ...
    document.getElementById("my_mess_cuts").innerHTML =
      "<ion-spinner></ion-spinner>";
...
function changePassword(user) {
  ...
  if (newPassword != confirmation) {
    errors.innerHTML =
      "<ul><li style='color: red;'>Passwords dont match</li></ul>";
    document.getElementById("change_button").innerHTML = "Change";
  } else {
    post(
      "/" + user + "/changepassword",
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (myObj.length == 0) {
        ...
        for (x in myObj) {
          ...
          txt +=
            "<ion-item class='student_item_" +
            id +
            "' lines='none' detail='true' href=# onClick='" +
            functionCall +
...
function check(student_id, id, el) {
  if (el.previousSibling.value == "1") {
    ...
    el.innerHTML =
      "GET DETAILED FEE<ion-icon slot='end' name='chevron-down'></ion-icon>";
  } else {
    el.innerHTML =
      "GET DETAILED FEE<ion-icon slot='end' name='chevron-up'></ion-icon'";
...
function onGetFee(data, id) {
  ...
  txt +=
    ...
    MonthBill +
    "</ion-col></ion-item><ion-item><ion-col>Guest Fee</ion-col><ion-col>" +
    GuestFee +
    "</ion-col></ion-item><ion-item><ion-col>Extra Fee</ion-col><ion-col>" +
    ExtraFee +
    "</ion-col></ion-item><ion-item lines=none><ion-col><strong>Total Month Bill</strong></ion-col><ion-col><strong>" +
    TotalMonthFee +
    "</strong></ion-col></ion-item></ion-list></ion-card-container></ion-card>";
  document.getElementById(id).innerHTML = txt;
}
...
function getFeeList() {
  ...
  xmlhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      ExtraFee = TotalAmt;
      TotalMonthFee =
        TotalAmt + 80 * 30 + NoOfGuests * 80 - 80 * TotalNoOfMessCuts;
      txt +=
        "<ion-row><ion-col size=2 offset=2><ion-card><ion-card-container><ion-list><ion-item><ion-col>Month Fee</ion-col><ion-col>" +
        MonthBill +
        "</ion-col></ion-item><ion-item><ion-col>Guest Fee</ion-col><ion-col>" +
        GuestFee +
        "</ion-col></ion-item><ion-item><ion-col>Extra Fee</ion-col><ion-col>" +
        ExtraFee +
        "</ion-col></ion-item><ion-item><ion-col>Total Month Bill</ion-col><ion-col>" +
        TotalMonthFee +
        "</ion-col></ion-item></ion-list></ion-card-container></ion-card></ion-col></ion-row>";
...
function filterMessCuts() {
  ...
  for (i = 0; i < items.length; i++) {
    ...
    if (
      filterDate.getTime() !== filterDate.getTime() ||
